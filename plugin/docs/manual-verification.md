...
环境：DSH 0.1.2-alpha.1 宿主 + 本插件 `npm pack` 产物装入 profile。控制台确认 `[dsh.turnfold] v0.3.4 loaded`。
...
- [ ] 收起**全程及终局无跳动**：选择包含多条过程成员行、且折叠条与回答之间夹有系统提示词行的轮次——收拢过程中与结束后，①折叠条下方的系统提示词行不应瞬移 8px（bar margin-bottom 渐变吸收）；②回答块自身不应瞬缩/瞬胀 40px（flip 提交 + ResizeObserver 追赶动画吸收）；③下方任意正文/下一轮内容不应抽动；④已收拢内容不回弹一帧
- [ ] 展开**起点无跳动**：点击展开瞬间，折叠条下方行不应瞬移、回答块不应瞬胀（bar margin-bottom 与 answer 形态与成员行同组渐变）
- [ ] 动画进行中反复快速连点：应从当前视觉进度连续反向，不应跳到端点重播；最后一次点击决定最终开合状态（flip 已触发后连点回展开，最终应停在展开态）
- [ ] 开启系统“减少动态效果”后：开合退化为瞬时切换，连续点击不丢失展开意图
