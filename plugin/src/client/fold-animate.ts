 ...
 * `hidden="until-found"`（React layout effect 子先于父），所以展开方向必须
 * 等布局就绪再测量——hidden 摘除、offsetHeight 非 0 后才锁定 0 高、下一帧
 * 展开，否则会把 0 当目标（0→0 无过渡 = 内容消失）。收起方向行本就可见，
 * 首帧同步测量。测量不到自然高时放弃动画（回退官方瞬间切换）。
 *
 ...
 * 展开方向用 setTimeout 轮询（而非 requestAnimationFrame）：IAB 后台/未激活
 * 面板里 rAF 停发，依赖它轮询会永远测不到就绪；setTimeout 在后台照常触发，
 * hidden 摘除后最多等 ~3 帧即可测到。仍测不到就放弃，绝不把 0 当目标。
 */
...
export function animateFoldRows(
  ...
  const runAnimation = (measures: RowMeasure[]): void => {
    for (let i = 0; i < rows.length; i += 1) {
      ...
      const anim = deps.animate(el, [
        { height: `${fromHeight}px`, marginTop: `${fromMargin}px`, opacity: expand ? '0' : '1' },
  ...
  const begin = (): void => {
    ...
    // 测量阶段的兜底由 measureWhenReady 自带（轮询至多 3 次后 resolve([])，
    // 触发 settle）——不再额外挂 timeout，避免与轮询在假时序里互相覆盖。
